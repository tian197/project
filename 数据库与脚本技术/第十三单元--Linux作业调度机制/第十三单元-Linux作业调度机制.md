...
计划任务，在特定的时间执行某项工作，在特定的时间执行一次，需要安装at服务，`yum -y install at`



### 13.1.1 at应用场景

在未来的某个时间点执行一次某个任务，如果在企业管理中，有一个任务要在以后的一个时间里运行一次，那就使用at命令设置任务执行时间与执行的任务便可，如果任务比较复杂，可将命令写成脚本并在at命令中调用
...
（1）中断进程 
使用ctrl+c组合键可以生产SIGINT信号比如用sleep命令测试 
...
（2）暂停进程 
有些进程想暂停而不是终止它可以使用ctrl+z组合键生产SIGTSTP信号 
